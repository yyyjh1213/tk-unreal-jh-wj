...
class SceneOperation(HookBaseClass):
    ...
    def _save_clean_file(self, file_path):
        """
        Clean 모드로 파일을 저장하는 헬퍼 함수
        ...
        if ".v" in name:
            ...
            clean_name = f"{base_name}.clean.v{version}{ext}"
        else:
            # 버전 번호가 없는 경우 처리
            clean_name = f"{name}.clean{ext}"
        
        # clean 폴더에 파일 저장
        clean_path = os.path.join(clean_folder, clean_name)
        cmds.file(rename=clean_path)
        # mayaAscii 형식으로 강제 저장
    ...
    def execute(self, operation, file_path, context, parent_action, file_version, read_only, **kwargs):
        ...
        if operation == "open":
            ...
            if clean_enabled:
                self._save_clean_file(file_path)
            else:
                # 일반 저장
